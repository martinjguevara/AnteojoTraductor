...
class detector_manos():
    def __init__(self, mode=False, maxManos = 2, model_complexity=1, Confdetection = 0.5, Confsegui = 0.5):
        self.mode = mode
        ...
        self.manos = self.mpmanos.Hands(self.mode, self.maxManos, self.compl, self.Confdetection, self.Confsegui)
    ...
    def dedosArriba(self):
        dedos = []
        if self.list[self.tip[0]][1] > self.lista[self.tip[0]-1][1]:
            dedos.append(1)
        else:
            dedos.append(0)
            
        for id in range (1, 5):
            if self.list[self.tip[id]][2] < self.lista[self.tip[id]-2][2]:
                dedos.append(1)
            else:
                dedos.append(0)
